...
namespace Microsoft.CLU.CommandPackage
{
    ...
    public class CommandPackage
    {
        public static int Handle(PackageConfig config, DirectoryInfo baseDirectory, string[] args)
        {
            ...
            else if (freeForAllArguments.Contains("--getNextWordMatches"))
            {
                freeForAllArguments.Remove("--getNextWordMatches");
                command = (cfg, dir, remainingArgs) =>
                {
                    foreach (var word in GetNextWordMatches(cfg, dir, remainingArgs))
                    {
            ...
            else if (freeForAllArguments.Contains("--getMatchingCommands"))
            {
                freeForAllArguments.Remove("--getMatchingCommands");
                command = (cfg, dir, remainingArgs) =>
                {
                    foreach (var cmd in GetMatchingFullCommandStrings(cfg, dir, remainingArgs))
                    {
        ...
        public static IEnumerable<string> GetMatchingFullCommandStrings(PackageConfig config, DirectoryInfo baseDirectory, string[] args)
        {
            return GetMatchingCommands(config, baseDirectory, true, args)
                    .Select((cmdlet) => { return String.Join(" ", cmdlet.CommandDiscriminators); });
        }

        public static IEnumerable<string> GetNextWordMatches(PackageConfig config, DirectoryInfo baseDirectory, string[] args)
        {
            return GetMatchingCommands(config, baseDirectory, false, args)
                    .Select((cmdlet) => { return cmdlet.CommandDiscriminators.Last(); });
        }

        private static IEnumerable<Common.CmdletValue> GetMatchingCommands(PackageConfig config, DirectoryInfo baseDirectory, bool matchPartialCommands, string[] args)
        {
            var package = new Common.CmdletLocalPackage(config, baseDirectory);

            return package.FindMatchingCommandlets(args, matchPartialCommands);
        }
